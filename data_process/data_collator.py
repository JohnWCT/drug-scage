...
atom_categorical_names = list(CompoundKit.atom_vocab_dict.keys())
atom_float_names = list(CompoundKit.atom_float_names)
...
def collator_finetune_pkl(items: List[Dict[str, Union[List[int], np.ndarray, str, int]]]):
    ...
    for item in items:
        ...
        for name in atom_categorical_names:
            raw_arr = np.asarray(item[name])
            raw_arr = np.nan_to_num(raw_arr, nan=-1.0, posinf=-1.0, neginf=-1.0)
            max_idx = CompoundKit.get_atom_feature_size(name) + 3
            safe_arr = np.clip(raw_arr.astype(np.int64), 0, max_idx)
            item[name] = padding_1d_sequence(safe_arr, max_len, -1) + 1
        for name in atom_float_names:
            float_arr = np.asarray(item[name], dtype=np.float32)
            float_arr = np.nan_to_num(float_arr, nan=0.0, posinf=0.0, neginf=0.0)
            item[name] = padding_1d_sequence(float_arr, max_len, 0)
