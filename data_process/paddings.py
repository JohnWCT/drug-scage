...
def padding_pair_distances_loong(pair_distances, edges, max_length, padding_value=0, item=0):
    ...
    for i in range(len(edges)):
        # Some generated PKL samples may store edge indices as float dtype.
        # Convert to int explicitly and skip malformed/out-of-range values.
        try:
            src = int(edges[i][0])
            dst = int(edges[i][1])
            if src < 0 or dst < 0:
                continue
            if src >= len(pair_distances) or dst >= len(pair_distances[src]):
                continue
            matrix[i] = pair_distances[src][dst]
