...
class Trainer(object):
    ...
    def train(self):
        ...
        valid_out = self._valid_step(self.test_loader)

        if self.config['task'] == 'regression':
            test_loss, test_mae, test_rmse, test_r2 = valid_out
        else:
            test_loss, test_auc, test_pr_auc = valid_out

        test_metric_list.append(test_rmse if self.config['task'] == 'regression' else test_auc)

        task_name = config['task_name']
        if config['task'] == 'classification':
            print(f'{task_name} test_auc:{test_auc} test_pr_auc:{test_pr_auc}')
        else:
            print(f'{task_name} test_mae:{test_mae} test_rmse:{test_rmse} test_r2:{test_r2}')
...
def parse_args():
    ...
    parser.add_argument('--seed', default=9, type=int,
                        help='split seed for random_scaffold (default: 9)')
