...
def get_balanced_atom_fg_loss(pred__, label__, loss_f_atom_fg: Callable, advanced=True):
    if not advanced:
        ...
        if table__.numel() == 0:
            w_positive = torch.ones_like(indices_k, dtype=torch.float32, device='cuda')
        else:
            safe_indices_k = torch.clamp(indices_k, min=0, max=table__.numel() - 1)
            w_positive = table__[safe_indices_k]
            invalid_mask = (indices_k < 0) | (indices_k >= table__.numel())
            if torch.any(invalid_mask):
                w_positive[invalid_mask] = 1.0
        w_negative = torch.ones_like(w_positive)
