...
def compute_cls_metric_tensor_with_pr(y_true: Tensor, y_pred: Tensor):
    y_true = y_true.view(y_pred.shape)
    dataset_form: str = get_dataset_form()
    if dataset_form == 'pyg':
        is_valid = y_true ** 2 > 0
    ...
    for i in range(y_true.shape[1]):
        ...
        if len(np.unique(label_np)) == 2:
            roc_list.append(roc_auc_score(label_np, pred_np))
            precision, recall, _ = precision_recall_curve(label_np, pred_np)
            pr_list.append(auc(recall, precision))
        else:
            loguru.logger.warning(f"No positively labeled pretrain_data available for label {i}.")
    if len(roc_list) == 0:
        raise RuntimeError("No positively labeled pretrain_data available. Cannot compute ROC-AUC/PR-AUC.")
...
def compute_reg_metric_with_r2(y_true: Tensor, y_pred: Tensor):
    mae, rmse = compute_reg_metric(y_true, y_pred)
    r2_list = []
    for i in range(y_true.shape[1]):
        label, pred = y_true[:, i], y_pred[:, i]
        ...
        if torch.isclose(ss_tot, torch.tensor(0.0, device=ss_tot.device)):
            r2 = torch.tensor(0.0, device=ss_tot.device)
        else:
            r2 = 1.0 - (ss_res / ss_tot)
        r2_list.append(r2)
    r2 = torch.mean(torch.stack(r2_list)).item()
    return mae, rmse, r2
