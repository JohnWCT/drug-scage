...
def get_finetune_log_dir(config: Dict) -> str:
    """Use a stable run directory (no timestamp) for pipeline reproducibility."""
    run_name = f"{config['task_name']}_{config['seed']}_{config['split_type']}_{config['optim']['init_lr']}_{config['batch_size']}"
    return os.path.join("finetune_result", run_name)


def create_fresh_writer(config: Dict) -> SummaryWriter:
    log_dir = get_finetune_log_dir(config)
...
class Trainer(object):
    def __init__(self, config, file_path):
        ...
        if config['checkpoint'] and GlobalVar.use_ckpt:
            ...
            self.writer = create_fresh_writer(self.config)
    ...
    def _save_learning_curve(self, epoch_list, train_loss_list, valid_loss_list, test_loss_list,
        ...
        if not epoch_list:
            return
        metric_label = 'AUC' if self.config['task'] == 'classification' else 'RMSE/MAE'
        ...
        plt.subplot(1, 2, 2)
        plt.plot(epoch_list, train_metric_list, label='train_metric')
        plt.plot(epoch_list, val_metric_list, label='valid_metric')
        plt.plot(epoch_list, test_metric_list, label='test_metric')
        plt.xlabel('Epoch')
...
def parse_args():
    ...
    parser.add_argument('--pin_memory', type=int, default=0,
                        help='whether to use pin_memory in dataloader: 0/1 (default: 0)')
...
def main(config):
    ...
    if args.task == 'sider':
        args.batch_size = 10
    # Keep historical auto-patience behavior only when user keeps default.
    if (args.task in REGRESSION_TASK_NAMES or args.task == 'clintox') and args.patience == 10:
        args.patience = 30
    ...
    config['save_ckpt'] = max(1, int(args.save_ckpt))
    ...
    config['lr_scheduler']['warm_up_epoch'] = args.warm_up_epoch
    ...
    # Keep checkpoint saving enabled so downstream pipeline can collect model.pth.
